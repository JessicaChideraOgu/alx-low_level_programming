...
void print_magic(unsigned char *e_ident)
{
	...
	for (buffer = 0; buffer < EI_NIDENT; buffer++)
	{
		printf("%02x", e_ident[buffer]);

		if (buffer == EI_NIDENT - 1)
		printf("\n");
		else
		printf(" ");
	}
 ...
 * @e_ident: it pointers to an array containing the ELF version.
 */
void print_version(unsigned char *e_ident)
{
	printf(" Version: %d",
